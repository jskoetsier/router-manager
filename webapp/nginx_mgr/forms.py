...
class NginxProxyConfigForm(forms.ModelForm):
    ...
    class Meta:
        ...
        widgets = {
            ...
            "description": forms.Textarea(
                attrs={
                    "rows": 3,
                    "placeholder": "Optional description of this proxy configuration",
                }
            ),
            "domain_name": forms.TextInput(attrs={"placeholder": "e.g., example.com"}),
            "upstream_host": forms.TextInput(
                attrs={"placeholder": "127.0.0.1 or backend-server.local"}
            ),
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.instance.pk and self.instance.custom_headers:
            self.fields["custom_headers_text"].initial = json.dumps(
                self.instance.custom_headers, indent=2
            )
        ...
        self.helper.layout = Layout(
            HTML(
                '<h4 class="mb-3"><i class="fas fa-server"></i> Basic Configuration</h4>'
            ),
            Div(
                Row(
            ...
            HTML(
                '<h4 class="mb-3"><i class="fas fa-globe"></i> Domain & SSL Settings</h4>'
            ),
            Div(
                "domain_name",
                Row(
            ...
            HTML(
                '<h4 class="mb-3"><i class="fas fa-arrow-right"></i> Upstream Server</h4>'
            ),
            Div(
                Row(
            ...
            Div(
                Row(
                    Column(
                        AppendedText("proxy_read_timeout", "seconds"),
                        css_class="col-md-4",
                    ),
                    Column(
                        AppendedText("proxy_connect_timeout", "seconds"),
                        css_class="col-md-4",
                    ),
                    Column(
                        AppendedText("proxy_send_timeout", "seconds"),
                        css_class="col-md-4",
                    ),
                ),
                css_class="mb-4",
            ...
            HTML(
                '<h4 class="mb-3"><i class="fas fa-shield-alt"></i> Security & Rate Limiting</h4>'
            ),
            Div(
                Row(
                    Column("rate_limit_enabled", css_class="col-md-6"),
                    Column(
                        AppendedText("rate_limit_rpm", "req/min"), css_class="col-md-6"
                    ),
                ),
                css_class="mb-4",
            ),
            HTML('<h4 class="mb-3"><i class="fas fa-code"></i> Custom Headers</h4>'),
            Div("custom_headers_text", css_class="mb-4"),
            ...
            Div(
                Submit(
                    "save",
                    "Save Configuration",
                    css_class="btn btn-primary btn-lg me-2",
                ),
                HTML(
                    '<a href="{% url \'nginx_mgr:list\' %}" class="btn btn-secondary btn-lg">Cancel</a>'
                ),
        ...
        try:
            ...
            if not isinstance(headers, dict):
                raise ValidationError(
                    "Custom headers must be a JSON object (dictionary)"
                )
...
class QuickProxyForm(forms.Form):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.helper.layout = Layout(
            HTML(
                '<div class="alert alert-info"><i class="fas fa-info-circle"></i> Quick setup for a simple reverse proxy with automatic SSL</div>'
            ),
            "name",
            "domain_name",
            ...
            Div(
                Submit(
                    ...
                    css_class="btn btn-success btn-lg me-2",
                ),
                HTML(
                    '<a href="{% url \'nginx_mgr:list\' %}" class="btn btn-secondary btn-lg">Cancel</a>'
                ),
...
class DeployConfigForm(forms.Form):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.helper.layout = Layout(
            HTML(
                '<div class="alert alert-warning"><i class="fas fa-exclamation-triangle"></i> This will deploy the configuration to nginx and restart the service if needed.</div>'
            ),
            "force_reload",
            Div(
                Submit(
                    ...
                    css_class="btn btn-warning btn-lg me-2",
                ),
                HTML(
