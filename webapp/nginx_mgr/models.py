...
class NginxProxyConfig(models.Model):
    ...
    auto_ssl = models.BooleanField(default=True, help_text="Automatically obtain SSL certificates via Let's Encrypt")
    force_https = models.BooleanField(default=True, help_text="Redirect HTTP to HTTPS")
    
    # Upstream server settings
    upstream_host = models.CharField(max_length=255, default='127.0.0.1', help_text="Backend server host")
    upstream_port = models.PositiveIntegerField(default=8000, help_text="Backend server port")
    ...
    # Proxy settings
    proxy_read_timeout = models.PositiveIntegerField(default=60, help_text="Proxy read timeout in seconds")
    proxy_connect_timeout = models.PositiveIntegerField(default=60, help_text="Proxy connect timeout in seconds")
    proxy_send_timeout = models.PositiveIntegerField(default=60, help_text="Proxy send timeout in seconds")
    
    # Additional headers
    custom_headers = models.JSONField(default=dict, blank=True, help_text="Custom headers to add")
    
    # Rate limiting
    rate_limit_enabled = models.BooleanField(default=False, help_text="Enable rate limiting")
    rate_limit_rpm = models.PositiveIntegerField(default=100, help_text="Requests per minute limit")
    
    # Access control
    access_log_enabled = models.BooleanField(default=True, help_text="Enable access logging")
    error_log_enabled = models.BooleanField(default=True, help_text="Enable error logging")
    
    # Configuration state
    is_active = models.BooleanField(default=False, help_text="Whether this configuration is active")
    is_deployed = models.BooleanField(default=False, help_text="Whether this configuration is deployed")
...
class SSLCertificate(models.Model):
    ...
    # Certificate details
    certificate_path = models.CharField(max_length=500, help_text="Path to certificate file")
    private_key_path = models.CharField(max_length=500, help_text="Path to private key file")
    fullchain_path = models.CharField(max_length=500, help_text="Path to fullchain file")
    ...
    issuer = models.CharField(max_length=255, default="Let's Encrypt", help_text="Certificate issuer")
    
    # Status
    is_valid = models.BooleanField(default=True, help_text="Whether the certificate is valid")
    auto_renewal = models.BooleanField(default=True, help_text="Auto-renew this certificate")
...
class NginxDeploymentLog(models.Model):
    ...
    # Log details
    message = models.TextField(blank=True, help_text="Deployment message or error details")
