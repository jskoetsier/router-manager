...
def get_strongswan_status():
    ...
    try:
        ...
        result = run_command(['/usr/bin/sudo', '/usr/bin/systemctl', 'is-active', 'strongswan'])
...
def create_ipsec_config(tunnel_name, local_ip, remote_ip, local_subnet, remote_subnet, psk):
...
'''
        ...
        results.append(('Add ipsec.conf entry', result['success'], result.get('error', '')))
...
'''
        ...
        results.append(('Add ipsec.secrets entry', result['success'], result.get('error', '')))
        
        # Reload StrongSwan configuration
        result = run_command(['/usr/bin/sudo', '/usr/sbin/ipsec', 'reload'])
        results.append(('Reload StrongSwan', result['success'], result.get('error', '')))
        
        # Start the tunnel
        result = run_command(['/usr/bin/sudo', '/usr/sbin/ipsec', 'up', tunnel_config.get('name', 'tunnel')])
...
'''
        ...
        os.chmod(script_path, 0o755)
        result = run_command(['/usr/bin/sudo', '/bin/bash', script_path])
        results.append(('Remove ipsec.conf entry', result['success'], result.get('error', '')))
...
def get_tunnel_status(tunnel_name):
    ...
    try:
        result = run_command(['/usr/bin/sudo', '/usr/sbin/ipsec', 'status', tunnel_name])
        if result['success']:
