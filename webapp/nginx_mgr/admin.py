...
from .models import NginxDeploymentLog, NginxProxyConfig, SSLCertificate
...
class NginxProxyConfigAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ("Basic Configuration", {"fields": ("name", "description", "is_active")}),
        (
            "Domain & SSL Settings",
        ...
        ("Custom Headers", {"fields": ("custom_headers",), "classes": ("collapse",)}),
        (
            "Logging",
            {
    ...
    def ssl_status(self, obj):
        ...
        if not obj.ssl_enabled:
            return format_html(
                '<span class="badge badge-secondary">SSL Disabled</span>'
            )
        ...
        if obj.auto_ssl:
            if hasattr(obj, "ssl_certificate"):
                ...
                if cert.is_expiring_soon():
                    return format_html(
                        '<span class="badge badge-warning">SSL Expiring Soon</span>'
                    )
                else:
                    return format_html(
    ...
    def deploy_selected(self, request, queryset):
        ...
        for config in queryset:
            if not config.is_deployed:
                ...
                pass
        self.message_user(
            request, f"Deployment initiated for {queryset.count()} configurations."
        )
...
class SSLCertificateAdmin(admin.ModelAdmin):
    ...
    def expiry_status(self, obj):
        ...
        if days_left < 0:
            ...
            return format_html(
                '<span class="badge badge-danger">{} days left</span>', days_left
            )
        elif days_left < 30:
            return format_html(
                '<span class="badge badge-warning">{} days left</span>', days_left
            )
        else:
            return format_html(
                '<span class="badge badge-success">{} days left</span>', days_left
            )
...
class NginxDeploymentLogAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ("Deployment Information", {"fields": ("proxy_config", "action", "status")}),
        ("Timing", {"fields": ("started_at", "completed_at", "duration")}),
        (
            "Details",
            {"fields": ("message", "config_snapshot"), "classes": ("collapse",)},
        ),
    )
