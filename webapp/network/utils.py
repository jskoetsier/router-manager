...
def get_ip_forwarding_status():
    ...
    try:
        ...
        result = run_command(['/usr/bin/sudo', '/usr/sbin/sysctl', 'net.ipv4.ip_forward'])
        if result['success']:
        ...
        result = run_command(['/usr/bin/sudo', '/usr/sbin/sysctl', 'net.ipv6.conf.all.forwarding'])
        if result['success']:
...
def set_ip_forwarding(ipv4_enabled=True, ipv6_enabled=True, permanent=True):
    ...
    result = run_command(['/usr/bin/sudo', '/usr/sbin/sysctl', '-w', f'net.ipv4.ip_forward={ipv4_value}'])
    results.append(('IPv4 forwarding', result['success'], result.get('error', '')))
    
    # Set IPv6 forwarding
    ipv6_value = '1' if ipv6_enabled else '0'
    result = run_command(['/usr/bin/sudo', '/usr/sbin/sysctl', '-w', f'net.ipv6.conf.all.forwarding={ipv6_value}'])
    ...
    if permanent:
        try:
            ...
            backup_result = run_command(['/usr/bin/sudo', '/bin/cp', '/etc/sysctl.conf', '/etc/sysctl.conf.bak'])
...
'''
            ...
            result = run_command(['/usr/bin/sudo', '/bin/bash', '/tmp/update_sysctl.sh'])
            ...
            results.append(('Permanent configuration', result['success'], result.get('error', '')))
...
def configure_basic_nat(interface='eth0', enabled=True):
    ...
    if enabled:
        ...
        result = run_command([
            '/usr/bin/sudo', '/usr/sbin/nft', 'add', 'chain', 'ip', 'nat', 'postrouting', 
        ...
        result = run_command([
            '/usr/bin/sudo', '/usr/sbin/nft', 'add', 'rule', 'ip', 'nat', 'postrouting', 
            'oifname', interface, 'masquerade'
        ])
        results.append(('NAT masquerade rule', result['success'], result.get('error', '')))
        
    else:
        # Remove NAT table (this removes all NAT rules)
        result = run_command(['/usr/bin/sudo', '/usr/sbin/nft', 'delete', 'table', 'ip', 'nat'])
